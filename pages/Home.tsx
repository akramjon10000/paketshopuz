...
const Home = () => {
  ...
  const categories = [
    ...
    { id: 'boxes', name: lang === 'ru' ? 'Коробки' : 'Qutilar', icon: <Box size={32} /> },
    { id: 'disposable', name: lang === 'ru' ? 'Одноразовое' : 'Bir martalik', icon: <Package size={32} /> },
    { id: 'tape', name: lang === 'ru' ? 'Скотч' : 'Skotch', icon: <Scissors size={32} /> },
  ...
  const brands = [
    { name: 'OshMarkaz', logo: 'https://cdn-icons-png.flaticon.com/512/3214/3214150.png' },
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-1 gap-4 h-full">
            <div className="relative rounded-3xl overflow-hidden shadow-sm group cursor-pointer bg-slate-100 h-[160px] md:h-auto" onClick={() => navigate('/menu', { state: { category: 'boxes' } })}>
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <h3 className="font-black text-xl md:text-2xl uppercase">QUTILAR</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={index} className="bg-white border border-slate-100 p-4 rounded-2xl flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-4">
            {categories.map((category) => (
              <button
                key={category.id}
                onClick={() => navigate('/menu', { state: { category: category.id } })}
              ...
              >
                ...
                <span className="text-sm font-bold text-slate-700 group-hover:text-orange-600">{category.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-slate-200">
          ...
          <div className="flex flex-wrap justify-center md:justify-between items-center gap-8 md:gap-12 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
            {brands.map((brand, index) => (
              <img key={index} src={brand.logo} alt={brand.name} className="h-8 md:h-10 object-contain hover:scale-110 transition-transform cursor-pointer" />
            ))}
          </div>
        </div>
      </div>
